...
def display_single_report(request, slug):
    ...
    theorical_costs = display_chart_transporter(report=report)[1]
    total_calculated_costs = display_chart_transporter(report=report)[2]
    total_supplements = display_chart_transporter(report=report)[3]
    total_discrepancies = display_chart_transporter(report=report)[4]
    ...
    for transporter in transporters:
        ...
        if not TransporterFile.objects.filter(report=report, transporter=transporter_id).exists():
            ...
            # transporter_file = TransporterFile.objects.get(report=report, transporter=transporter_id)
        ...
        if 'transporter_form' in request.POST:
            ...
            if transporter_form.is_valid():
                ...
                report_results = ReportResults.objects.get_or_create(report=report, transporter=supplement.transporter,
                                                                     ...
                                                                     total_theorical_prices=comparison_results[1], total_calculated_prices=comparison_results[2],
                                                                     total_supplements=comparison_results[3], total_discrepancies=comparison_results[4])
    ...
    return render(request, 'reports/single-report.html',
                  context={'company_form': company_form, 'transporter_form': transporter_form,
                           'transporters': transporters, 'check_company_file': check_company_file,
                           'company_file_form': company_file_form, 'missing_transporters': missing_transporters,
                           'transporter_file_form': transporter_file_form, 'result_supplements': result_supplements,
